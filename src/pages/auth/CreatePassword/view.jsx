...
import { AlertCircle, CheckCircle2, ChevronLeft, KeyRound, LockKeyhole, Mail, ShieldCheck } from "lucide-react";
      ...
      <div className="hidden md:flex flex-col justify-between w-5/12 bg-[var(--color-primary-900)] p-12 text-white relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-slate-400 text-lg leading-relaxed max-w-sm">
            Create your password once, then use your email to sign in to SHNOOR LMS.
          </p>
        </div>
        ...
        <div className="relative z-10">
          <div className="bg-white/5 p-6 rounded-2xl border border-white/10 backdrop-blur-sm">
            <div className="flex gap-1 text-emerald-400 mb-2">
              <ShieldCheck size={20} />
              <span className="text-xs font-bold uppercase tracking-wider">Secure Firebase Flow</span>
            </div>
            <p className="text-slate-300 text-sm italic">Your password is stored in Firebase Auth. No password column was added to the database.</p>
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-12 relative bg-white">
        <div className="w-full max-w-[420px]">
          <div className="mb-10 text-center md:text-left">
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mb-2">Create Password</h1>
            <p className="text-slate-500 text-sm">Finish setting up your account, then continue to the login page.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="block text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">New Password</label>
              <div className="relative">
            ...
            <div className="space-y-2">
              <label className="block text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Confirm Password</label>
              <div className="relative">
                <LockKeyhole className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                <input
                  type="password"
            ...
            >
              {validatingLink ? "Validating Link..." : loading ? "Saving Password..." : "Create Password"}
            </button>
